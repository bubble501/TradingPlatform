...
import websocket
from datapool.general_config import zb_ws
from utilPool.wsUtil import wsUtilfunc


class QuoinexApi(wsUtilfunc):
    """基于Websocket的API对象"""
    ...
    def __init__(self):
        ...
        self.secretKey = ''  # 密码
        self.threadDict = {}
        self.host = zb_ws  # 服务器地址

    def onMessage(self, ws, evt):
        """信息推送"""
        data = self.readData(evt)
        if json.loads(evt).get('dataType')=='depth':
            self.res.append(json.dumps(json.loads(evt)))
